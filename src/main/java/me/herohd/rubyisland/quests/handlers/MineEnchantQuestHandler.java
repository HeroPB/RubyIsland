...
public class MineEnchantQuestHandler implements IQuestHandler {

    @Override
    public String getQuestType() {
        return "MINE_BREAK_ENCHANT";
    }

    @Override
    public void handleEvent(Event event, Player player, Quest quest, PlayerQuestData data, QuestManager manager) {
        if (!(event instanceof WMultipleBlockBreakEvent)) {
            return;
        }

        WMultipleBlockBreakEvent e = (WMultipleBlockBreakEvent) event;
        List<String> requiredBlock = quest.getRequireTarget();

        IRegion loc = RegionAPI.getRegionByLocation(e.getBroken().getLocation());
        if(loc == null) return;
        ...
        if (requiredBlock.contains("ANY") || requiredBlock.contains(mineName)) {
            manager.progressQuest(player, e.getBrokenAmount());
        }
    }
}