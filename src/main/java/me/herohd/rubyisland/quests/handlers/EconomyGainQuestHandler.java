...
public class EconomyGainQuestHandler implements IQuestHandler {
    @Override
    ...
    public void handleEvent(Event event, Player player, Quest quest, PlayerQuestData data, QuestManager manager) {
        if (!(event instanceof WEnchantGainObjectEvent)) {
            return;
        }

        WEnchantGainObjectEvent e = (WEnchantGainObjectEvent) event;
        List<String> requiredBlock = quest.getRequireTarget();
        if(requiredBlock.contains("ANY") || requiredBlock.contains(e.getType())) {
